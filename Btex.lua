...
function BtexSTRATADropDownMenu_Initialise()
	...
	info.value = 0; 
	info.func = BtexSTRATADropDownMenu_OnClick; 
...
function BtexSTRATADropDownMenuCUSTOM_Initialise()
	...
	info.text = "BACKGROUND(recommanded)"; 
	info.value = 0; 
	info.func = BtexSTRATADropDownMenuCUSTOM_OnClick; 
...
--------------------------------------------------------------------------------------------------------
-- frame
PanelGridConfigFrame = CreateFrame("Frame","PanelGridConfigFrame",BtexMenuFrame, BackdropTemplateMixin and "BackdropTemplate" );
...
function BtexDisplayGrid()
	...
	for i = 0, BtexGridBoxSize do 
		...
		if i == BtexGridBoxSize / 2 then BtexGridTexture:SetColorTexture(1, 0, 0, 0.5); 
		else BtexGridTexture:SetColorTexture(0, 0, 0, 0.5);
		end 
	...
	do
		local BtexGridTexture = BtexGrid:CreateTexture(nil, 'BACKGROUND') 
		BtexGridTexture:SetColorTexture(1, 0, 0, 0.5)
	...
	for i = 1, math.floor((GetScreenHeight()/2)/BtexGridHeightStep) do
		local BtexGridTexture = BtexGrid:CreateTexture(nil, 'BACKGROUND') 
		BtexGridTexture:SetColorTexture(0, 0, 0, 0.5)
